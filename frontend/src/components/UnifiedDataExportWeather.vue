  ...
  >
    <template #title>
      <span class="text-subtitle-1 font-weight-bold">
        Wetterdaten am Wohnort der Patient:innen
      </span>
    </template>

    <span class="text-caption">
      Die ausgewählten Wetterdaten werden verknüpft mit den URIs der
      Patient:innen-Einträge in Ihrer MII-Datei.
    </span>
  </v-list-item>
  <v-divider />
</template>
...
const matchData = () => {
  ...
  const patientsWithStations = props.miiData.patients.map((patient: any) => {
    ...
    return {
      patientId: patient.id,
      stationLatitude: nearestStation.station_latitude,
      stationLongitude: nearestStation.station_longitude,
    };
  });
    ...
    rows: exportData.data.rows
      .map((row) => {
        ...
        const matchingPatients = patientsWithStations.filter(
          (patient: any) =>
            patient.stationLatitude === row.station_latitude &&
            patient.stationLongitude === row.station_longitude
        );
  ...
  if (matchedData.rows.length === 0) {
    notificationsStore.addNotification(
      "Keine Patient:innen gefunden, die mit den Wetterdaten verknüpft werden können. Ändern Sie gegebenfalls die Filtereinstellungen."
    );
    return;
  }
